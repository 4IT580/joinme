...
export default function AccountInfo({ profileState }) {
  ...
  const profile = profileState.data.me
  ...
  return (
    <div className="rounded-3xl relative flex flex-col justify-center shadow-md bg-white p-6 py-4 mt-4 w-96">
      <div className="flex flex-col justify-center items-center">
        ...
        <span className={'text-lg text-gray-500'}>{city}</span>
      </div>
      ...
      <div className="flex flex-col mt-6">
        <span className={'font-bold uppercase text-sm'}>About {name}</span>{' '}
        <span className="pl-6 pt-2">{description}</span>
      </div>
      <div className="flex flex-col mt-6">
